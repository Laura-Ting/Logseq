...
- 去年考题
	...
	- 历史类
	  collapsed:: true
		...
		- Sora（2024.2）
		  collapsed:: true
			- Sora 是由 OpenAI 在 2024 年 2 月发布的一个文本到视频的生成模型，生成长达一分钟的 1080P 高清视频，扩散模型技术以及 Transformer 架构
	- 卷积类
	  collapsed:: true
		- 卷积的原理
		  collapsed:: true
			- 在深度学习中，卷积操作涉及一个被称为卷积核的小矩阵，该卷积核在输入数据（如图像）上滑动，计算卷积核和覆盖的输入数据部分的元素对应相乘后的总和，这个总和形成输出特征图的一个元素。通过这种方式，卷积核能够捕捉局部特征，并且通过不同的卷积核可以提取不同的特征，从而帮助神经网络识别和理解输入数据中的模式。
	...
	- bp类
	  collapsed:: true
		- 简述bp神经网络的理论
		  collapsed:: true
			- 反向传播（Backpropagation，简称BP）算法是一种在多层神经网络中训练权重的监督学习算法。它基于梯度下降法，通过前向传播计算输出误差，然后反向传播这些误差以调整网络中的权重，目的是最小化损失函数，从而使得网络能够准确预测或分类。BP算法利用了链式法则来计算每个权重对损失函数的贡献（即梯度），并据此更新权重，使网络在大量数据上学习到从输入到输出的复杂映射
		  ...
		  collapsed:: true
			...
			- 缺点：
			  collapsed:: true
				- 收敛速度慢
				  collapsed:: true
					- 本质为梯度下降法，它所要优化的目标函数非常复杂，因此会出现“锯齿形现象”，使得BP算法低效
				- 局部极值
				  collapsed:: true
					- BP算法可能会陷入局部最小值或鞍点，而不是全局最小值
				- 难以确定隐层和隐层结点的数目
	...
	- 搜索算法
	  collapsed:: true
		...
		- A的思路以及特性
		  collapsed:: true
			- 对OPEN表中的全部节点按估价值从小到大排序
			- A搜索算法不能保证找到最优解（路径最短的解）
		- A*步骤
		  collapsed:: true
			- 构造启发式函数h(n)，保证h(n)<h*(n)
			- 计算当前状态的f(n)=g(n)+h(n)，从候选项中选取最小值的选项继续迭代
			- 若有相同的最小值选项，取h(n)较小者，若h(n)也相等，则随机选择一个
		- A*和A有什么不同，特性
		  collapsed:: true
			- A*保证能找到最短路径
			- 可采纳性：当一个搜索算法在最短路径存在时能保证找到它，当h(n)=0时相当于宽度优先
			- 单调性：采用单调性启发函数，可以减少比较代价和调整路径的工作量，从而减少搜索代价
			  collapsed:: true
				- 对所有状态ni 和nj ，其中nj 是ni 的后裔，满足h(ni )h(nj)<=cost(ni, nj)，其中cost(ni, nj)是从ni到nj的实际代价
				- 目的状态的启发函数值为0
	...
	- 遗传算法
	  collapsed:: true
		...
		- 遗传算法的基本过程
		  collapsed:: true
			- 选择/复制：从当前群体中按照一定概率选出优良的个体，使它们有机会作为父代繁殖下一代子孙，选择的标准是适应度函数
		- 各个参数对算法的影响
		  background-color:: blue
		  collapsed:: true
			- 种群规模
			  collapsed:: true
				- 群体规模太小，遗传算法的优化性能不太好，易陷入局部最优解
				- 群体规模太大，计算复杂
			- 迭代次数：迭代次数是指遗传算法进行多少代的进化。迭代次数过少会导致搜索不充分，无法找到较好的解；迭代次数过多会导致计算时间增加，收敛速度变慢。
			- 交叉率：交叉率是指每代中进行交叉操作的个体比例。交叉操作是遗传算法中最重要的操作，它可以产生新的个体，增加种群的多样性，从而提高搜索能力。交叉率过低会导致搜索能力不足，陷入局部最优解；交叉率过高会导致搜索能力过强，破坏优秀个体，降低收敛速度。
			- 变异率：变异率是指每代中进行变异操作的基因比例。变异操作是遗传算法中辅助交叉操作的操作，它可以引入新的基因，增加种群的多样性，从而跳出局部最优解。变异率过低会导致搜索能力不足，陷入局部最优解；变异率过高会导致搜索能力过强，破坏优秀个体，降低收敛速度。
		- 适应度函数的尺度变换
		  collapsed:: true
			- 欺骗问题:所有妨碍适应度值高的个体产生，从而影响遗传算法正常工作的问题
			- 过早收敛：缩小这些个体的适应度，以降低这些超级个体的竞争力
			- 停滞现象：改变原始适应值的比例关系，以提高个体之间的竞争力
	- gan
		- gan简介：
		...
		- gan损失函数
			- ![Replaced by Image Uploader](https://raw.githubusercontent.com/qugushihua/blog-images/master/202406071546079.png)
	...
	- 模型的评估
		- 训练集和验证集的划分
			...
			- 自助法
			  collapsed:: true
				- 对数据集D有放回采样m次得到训练集D‘，D \ D'作为测试集
				- 实际模型与预期模型都使用m个训练样本
				- 约有1/3的样本没在训练集中出现，用作测试集 2:1
				- 从初始数据集中产生多个不同的训练集，对集成学习有很大的好处
